...
class SingleOutputTask(task.Task):
    ...
    def _load_statements(self, lines, split):
        base_path = "/var/local/statements_arxmliv_2018/"
        examples = []
        for (i, statement_path) in enumerate(lines):
            try:
                m = re.match(r"^\w+", statement_path)
                if (not (m.group(0) in REDUCED_STATEMENT_CLASSES)):
                    continue
                reduced_label = REDUCED_STATEMENT_CLASSES[m.group(0)]
                fh = open(os.path.join(base_path, statement_path), "r")
                statement = ' . '.join([line.rstrip()
                ...
                utils.log("Error constructing example from statement", i,
                          "for task", self.name + ":", ex)
                utils.log("Input causing the error:", statement_path)
        return examples
